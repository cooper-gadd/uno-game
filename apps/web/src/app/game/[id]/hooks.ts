  ...
  useEffect(() => {
    const connect = () => {
      ws.current = createWebSocketConnection(
        `${(env.NEXT_PUBLIC_WEBSOCKET_URL ?? "localhost:8080").includes("localhost") ? "ws://" : "wss://"}${env.NEXT_PUBLIC_WEBSOCKET_URL ?? "localhost:8080"}/game-chat?gameId=${gameId}`,
        (event: MessageEvent) => {
          try {
...
export function useGameUpdates(gameId: string) {
  ...
  useEffect(() => {
    const connect = () => {
      ws.current = createWebSocketConnection(
        `${(env.NEXT_PUBLIC_WEBSOCKET_URL ?? "localhost:8080").includes("localhost") ? "ws://" : "wss://"}${env.NEXT_PUBLIC_WEBSOCKET_URL ?? "localhost:8080"}/game-update?gameId=${gameId}`,
        (event: MessageEvent) => {
          try {
